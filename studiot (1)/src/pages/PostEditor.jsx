...
export default function PostEditor() {
  ...
  // Only admins really need all workspaces; but harmless because RLS will filter anyway
  ...
  const handleSave = (formData) => {
    if (postId) {
      ...
      // NOTE: your posts table does NOT have created_by email; rely on RLS + memberships.
  ...
  const handleMarkPosted = async () => {
    // Your schema has status but no posted_at/posted_by columns.
    await updateMutation.mutateAsync({ status: "posted" });

    // audit_logs schema: (workspace_id, entity_type, entity_id, action, actor_id, details)
    // We use actor_id = auth.uid() via profiles.id matching auth.users.id (your schema)
    if (post?.workspace_id) {
      const { error } = await supabase.from("audit_logs").insert({
        ...
        details: { action: "Marked as posted" },
      });
      if (error) {
        // don't block UX
