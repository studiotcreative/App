  ...
  const { data: allPosts = [], isLoading: loadingPosts } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("posts")
        .select(
          "id, workspace_id, social_account_id, platform, status, approval_status, caption, hashtags, first_comment, client_notes, asset_urls, asset_types, order_index, scheduled_date, scheduled_time"
        )
        .eq("workspace_id", clientWorkspaceId)
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <Button variant="outline" onClick={() => navigate(createPageUrl("ClientCalendar"))}>
      ...
      {isLoadingAny ? (
        ...
        <div className="bg-slate-50 rounded-2xl p-4 sm:p-6">
          ...
          <FeedPreviewGrid posts={posts} accounts={accounts} platform={selectedPlatform} isReadOnly={true} />
        </div>
      )}
    </div>
  );
